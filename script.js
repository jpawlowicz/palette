...
function generatePalette(baseColor) {
  const shades = [];
  for (let i = 1; i <= 9; i++) {
    const shade = baseColor.clone().lightness(100 - i * 10).hexString;
    shades.push(shade);
  }
  return shades;
}
...
function displayPalette(palette) {
  ...
  for (let i = 0; i < palette.length; i++) {
    ...
    card.className = "w-32 h-32 bg-white p-2 m-2 rounded shadow flex flex-col justify-between";
...
const inputField = document.getElementById("hex-input");
